...
public class Level1 extends Activity {
	 ...
	 * 记录循环数的该处是否有命令
	 */
	Boolean fB[] = { false, false, false, false, false };// 记录是否有命令
	...
	private void handleOK() {
		...
		new AlertDialog.Builder(Level1.this)
				// 这儿用getApplicationContext（）会崩溃
				.setMessage("恭喜你，成功完成了第一关。")
				...
				.setPositiveButton("让我们再来一遍",
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog,
									int which) {
								finish();
								Intent intentLevel1 = new Intent();
								intentLevel1.setClass(getApplicationContext(),
										Level1.class);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		game1 = (MyView1) findViewById(R.id.game1);
		...
		btnStartAndstop.setOnClickListener(new Button.OnClickListener() {
			public void onClick(View v) {
				if (btnStartAndstop.getText().equals("开始")) {
					...
					timer = new Timer();// timer被取消后，再使用都得重建
					TimerTask timerTask = new TimerTask() {// timerTask也一样
						@Override
						public void run() {
	 ...
	 */
	boolean isOK() {
		return Chongzi1.posi[0][0] == MyView1.x
				&& Chongzi1.posi[0][1] == MyView1.y && MyView1.bean == false;
	}
